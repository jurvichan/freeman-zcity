...
local colGray = Color(100,110,120,255)
local colBlue = Color(0,60,120)
local colBlueUp = Color(0,90,160)
local col = Color(255,255,255,255)

local colSpect1 = Color(15,20,35,255)
local colSpect2 = Color(20,30,50,255)

local colorBG = Color(10,15,25,220)
local colorBGBlacky = Color(8,12,20,235)
...
function GM:ScoreboardShow()
	...
	muteallbut:SetText("Hepsini Sustur")
	...
	mutespectbut:SetText("Seyircileri Sustur")
	...
	scoreBoardMenu.PaintOver = function(self,w,h)
		surface.SetDrawColor( 0, 100, 180, 220)
        surface.DrawOutlinedRect( 0, 0, w, h, 2.5 )
		...
		surface.SetFont( "ZB_InterfaceMediumLarge" )
		surface.SetTextColor(col.r,col.g,col.b,col.a)
		local lengthX, lengthY = surface.GetTextSize("Seyirciler:")
		surface.SetTextPos(w * 0.75 - lengthX/2,ScreenScale(25))
		surface.DrawText("Seyirciler:")
	...
	if LocalPlayer():Team() ~= TEAM_SPECTATOR then
		...
		SPECTATE.Paint = function(self,w,h)
			...
			surface.SetTextColor(col.r,col.g,col.b,col.a)
			local lengthX, lengthY = surface.GetTextSize("Katıl")
			surface.SetTextPos( lengthX - lengthX/2, 2)
			surface.DrawText("Katıl")
		end
	end
	...
	function DScrollPanel:Paint( w, h )
		...
		surface.SetDrawColor(15, 15, 20, 180)
	...
	for i, ply in player.Iterator() do -- надо это говно переделать.
		...
		but.Paint = function(self, w, h)
			...
			surface.DrawText(ply:Name() or "Çıktı...")
	
			surface.SetFont("ZB_InterfaceMediumLarge")
			surface.SetTextColor(col.r, col.g, col.b, col.a)
			local lengthX, lengthY = surface.GetTextSize(ply:Ping() or "He quited...")
			surface.SetTextPos(w - lengthX - 15, h / 2 - lengthY / 2)
			surface.DrawText(ply:Ping() or "Çıktı...")
		end

		function but:DoClick()
			if ply:IsBot() then chat.AddText(Color(255,0,0), "Bot profiline gidilemez.") return end
		...
		function but:DoRightClick()
			...
			local Menu = DermaMenu()
			Menu:AddOption( "Hesap", function(self)
				zb.Experience.AccountMenu( ply )
			end)
			Menu:AddOption( "SteamID Kopyala", function(self)
	...
	for i, ply in player.Iterator() do
		...
		function but:DoClick()
			if ply:IsBot() then chat.AddText("Bu bir bot.") return end
			gui.OpenURL("https://steamcommunity.com/profiles/"..ply:SteamID64())
		end
