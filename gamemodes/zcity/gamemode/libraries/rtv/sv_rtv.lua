...
-- Bu haritalar sunucuda varsa her zaman listeye dahil edilir
local forcedMaps = {
    "cs_apartments", "cs_christ_borough", "cs_market_2_gc",
    "de_donya",
    "gm_apartments_hl2", "gm_bbicotka_hmcd", "gm_building_v2", "gm_building_v2_war",
    "gm_church", "gm_cs_office_ext", "gm_csgosafehouse", "gm_eliden_hmcd",
    "gm_hmcd_rooftops", "gm_lakeview", "gm_militia_big", "gm_school",
    "gm_voidtown", "gm_z_trauma", "gm_zabroshka_winter",
    "hmcd_rooftops", "hmcd_rooftops_snow",
    "mu_abandoned", "mu_powerhermit", "mu_rascal", "mu_rascal_old",
    "ttt_clue_2022",
    "zs_cursed_woods_v3", "zs_lighthouse_revived_v1b", "zs_richland_remix_v1a",
}
...
local function getmaps()
    ...
    -- Zorunlu haritaları ekle (sunucuda varsa ve listede yoksa)
    for _, fmap in ipairs(forcedMaps) do
        if not table.HasValue(mappull, fmap) and file.Exists("maps/" .. fmap .. ".bsp", "GAME") then
            table.insert(mappull, fmap)
        end
    end
end
...
COMMANDS.forcertv = {function(ply, args)
	if not ply:IsAdmin() then ply:ChatPrint("Bu komuta erişiminiz yok.") return end
		zb.StartRTV(20)
	end,
	0
}
    ...
    if votes >= votesNeeded then
        if needPrint then
            for _, v in player.Iterator() do
                v:ChatPrint("Harita değiştirmek için yeterli oy var. RTV bir sonraki roundda başlayacak.")
            end
        end
...
COMMANDS.rtv = {function(ply, args)
    ...
    if rtvVotes[steamID] then
        ...
        ply:ChatPrint("Harita değiştirme oyunuzu iptal ettiniz.")
    ...
    for _, v in player.Iterator() do
        if remaining != 0 then
            v:ChatPrint(
                ply:Nick() .. " harita değiştirmek için oy verdi. " ..
                remaining .. " oy daha gerekiyor. İptal için tekrar !rtv yazın."
            )
        end
    end
