...
function MODE:Intermission()
    ...
    PrintMessage(HUD_PRINTTALK, "Raunt "..(self.Rounds - zb.RoundsLeft).."/"..self.Rounds)
...
concommand.Add("tdm_setrounds", function(ply, cmd, args)
    ...
    PrintMessage(HUD_PRINTTALK, "TDM raunt sayısı "..MODE.Rounds.." olarak ayarlandı. Kalan raunt: "..zb.RoundsLeft)
end)

COMMANDS.nextcsround = {
	function(ply, args)
		if not ply:IsAdmin() then ply:ChatPrint("Erişim izniniz yok") return end
		if string.lower(args[1]) == "bomb" then
            zb.nextcsround = "bomb"
            PrintMessage(HUD_PRINTTALK, "CS raundu seçildi - Bomba")
        end

        if string.lower(args[1]) == "hostage" then
            zb.nextcsround = "hostage"
            PrintMessage(HUD_PRINTTALK, "CS raundu seçildi - Rehine")
        end
	end,
	0
}
...
function MODE:EndRound()
    ...
    elseif zb.rtype == "hostage" then
        ...
        if IsValid(zb.hostage) and not zb.hostage.organism.alive then
            ...
            if zb.HarmDoneDetailed[zb.hostage:EntIndex()] then
                ...
                PrintMessage(HUD_PRINTTALK, (maxTeam == 0 and "Teröristler" or "Karşı-Teröristler") .. " rehineyi öldürdü")
            else
                winner = 3
            end
        end
    ...
    local winnerprt = (winner == 1 and "Karşı-Teröristler") or (winner == 0 and "Teröristler") or "Kimse"

    PrintMessage(HUD_PRINTTALK, winnerprt.." raundu kazandı.")
	...
	if winsTeam0 > winsTeam1 then
		for _, ply in ipairs(team.GetPlayers(1)) do
			...
			ply:ChatPrint("Takımınız kaybettiği için 1000 para tazminat aldınız.")
		end
    ...
    if zb.RoundsLeft > 0 then
        ...
        if winner then
            ...
            PrintMessage(HUD_PRINTTALK, winnerprt.." oyunu kazandı.")
        end
