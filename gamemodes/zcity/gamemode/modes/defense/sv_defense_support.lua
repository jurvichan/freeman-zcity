    ...
    if not dropPos then
        ...
        if spawnPoints and #spawnPoints > 0 then
            ...
            if IsValid(requester) then
                requester:ChatPrint("Teslimat için uygun yer bulunamadı. Daha sonra tekrar deneyin.")
            end
            return false
        end
    end
        ...
        if distToTarget < 10 then
            ...
            for _, player in player.Iterator() do
                if player:Alive() and player:Team() ~= TEAM_SPECTATOR then
                    if IsValid(requester) then
                        player:ChatPrint("Komutan " .. requester:Nick() .. "'nin ikmal paketi geldi!")
                    else
                        player:ChatPrint("Bir ikmal paketi geldi!")
                    end
                end
            end
        end
...
local function SpawnSupportTeam(requester)
    ...
    if #spawnPoints < 2 then
        if IsValid(requester) then
            requester:ChatPrint("Destek timi konuşlandırmak için yeterli alan yok!")
        end
        return false
    end
    ...
    if successfulSpawns > 0 then
        requester:ChatPrint("Destek timi " .. successfulSpawns .. " askerle konuşlandırıldı")
        

        for _, player in player.Iterator() do
            if player:Alive() and player:Team() != TEAM_SPECTATOR and player != requester then
                player:ChatPrint("Komutan " .. requester:Nick() .. " destek timi çağırdı!")
            end
        end
...
local function RespawnDeadPlayers(requester)
    ...
    if #deadPlayers == 0 then
        if IsValid(requester) then
            requester:ChatPrint("Yeniden doğurulacak ölü oyuncu yok!")
        end
        return false
    end
    ...
    if not spawnPoints or #spawnPoints == 0 then
        if IsValid(requester) then
            requester:ChatPrint("Mevcut doğuş noktası yok!")
        end
        return false
    end
    ...
    if respawnedCount > 0 then
        ...
        for _, player in player.Iterator() do
            player:ChatPrint("Komutan " .. requester:Nick() .. " takviye çağırdı! " .. respawnedCount .. " oyuncu yeniden doğdu!")
        end
...
net.Receive("RequestSupport", function(len, ply)
    ...
    ply:ChatPrint("#" .. orderId .. " siparişiniz yola çıktı!")
    ...
    if MODE and MODE.CreateTimer then
        MODE:CreateTimer(timerName, delay, 1, function()
            ...
            if not success and IsValid(ply) then
                ply:ChatPrint("Uygun düşüş konumu bulunamadı.")
            end
        end)
    end
end)
...
net.Receive("defense_commander_purchase", function(len, ply)
    ...
    if ply.organism and ply.organism.otrub then
        ...
        net.WriteString("Mevcut durumunuzda sipariş veremezsiniz!")
        net.WriteInt(0, 16)
        net.Send(ply)
        return
    end
    ...
    if totalCost > currentPoints then
        net.Start("defense_commander_notification")
        net.WriteString("Bu sipariş için yeterli ikmal puanı yok!")
        net.WriteInt(0, 16)
        net.Send(ply)
        return
    end
    ...
    net.Start("defense_commander_notification")
    net.WriteString("Sipariş başarıyla verildi! İkmal paketi yolda.")
        ...
        timer.Simple(5, function()
            ...
            if not success and IsValid(ply) then
                ...
                if refundAmount > 0 and IsValid(ply) then
                    ...
                    net.Start("defense_commander_notification")
                    net.WriteString("Uygun düşüş konumu bulunamadı. Normal ürünler için puanlar iade edildi.")
    ...
    if not specialsSuccess and IsValid(ply) then
        net.Start("defense_commander_notification")
        net.WriteString("Bazı özel ürünler işlenemedi. Kısmi iade yapıldı.")
