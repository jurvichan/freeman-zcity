...
local teams = {
	[1] = {
		objective = "Üssünüzü Combine saldırısından koruyun.",
		name = "Mülteci",
...
function MODE:HUDPaint()
	if NextWave_Time > CurTime() - 5 then
		...
		draw.SimpleText( "Sonraki dalga: ".. time.m ..":" .. time.s, "ZB_HomicideMedium", sw * 0.5, sh * (0.9 + timePos), Color(87,146,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
    ...
    draw.SimpleText("ZBattle | HL2 Üs Savunması", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

    local playerRole = lply:GetNWString("PlayerRole", "Mülteci")
    local roleColor = teams[team_].color1
    roleColor.a = 255 * fade
    draw.SimpleText("Rolünüz: " .. playerRole, "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.5, roleColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
local colGray = Color(30,30,35,255)
...
CreateEndMenu = function()
    ...
    hmcdEndMenu.PaintOver = function(self,w,h)
		...
		surface.SetTextColor(col.r,col.g,col.b,col.a)
		local lengthX, lengthY = surface.GetTextSize("Oyuncular:")
		surface.SetTextPos(w / 2 - lengthX/2,20)
		surface.DrawText("Oyuncular:")
	end
	...
	for i,ply in player.Iterator() do
		...
		but.Paint = function(self,w,h)
			...
			surface.SetFont( "ZB_InterfaceMediumLarge" )
			local lengthX, lengthY = surface.GetTextSize( ply:GetPlayerName() or "Ayrıldı..." )
			
			surface.SetTextColor(0,0,0,255)
			surface.SetTextPos(w / 2 + 1,h/2 - lengthY/2 + 1)
			surface.DrawText(ply:GetPlayerName() or "Ayrıldı...")
			...
			surface.DrawText((ply:Name() .. (not ply:Alive() and " - öldü" or "")) or "Ayrıldı...")

			surface.SetFont( "ZB_InterfaceMediumLarge" )
			surface.SetTextColor(col.r,col.g,col.b,col.a)
			local lengthX, lengthY = surface.GetTextSize( ply:Frags() or "Ayrıldı..." )
			surface.SetTextPos(w - lengthX -15,h/2 - lengthY/2)
			surface.DrawText(ply:Frags() or "Ayrıldı...")
		end
...
local function CreateVoteMenu()
    ...
    voteMenu.Paint = function(self, w, h)
        ...
        draw.SimpleText("OYUN MODU SEÇ", "Defense_Title", w / 2, 28, Color(230, 230, 230), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText("Bu raunt için oy kullanın", "Defense_Subtitle", w / 2, 55, Color(200, 200, 200), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    ...
    timerPanel.Paint = function(self, w, h)
        ...
        draw.SimpleText("KALAN SÜRE:", "Defense_Stats", w / 2, 12, Color(200, 200, 200), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText(timeLeft .. " SAN", "Defense_Timer", w / 2, 32, timeColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
...
MODE.HUDPaint = function(self)
    ...
    if showSelectedMode and CurTime() - selectedModeDisplayTime < selectedModeDisplayDuration then
        ...
        local text = "Seçilen mod: " .. modeName
...
local function DrawBossIncomingBanner()
    ...
    draw.SimpleText("PATRON GELİYOR", "ZB_HomicideMediumLarge", centerX + 2, textY + 2, shadowColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)


    draw.SimpleText("PATRON GELİYOR", "ZB_HomicideMediumLarge", centerX, textY, textColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)


    local infoY = y + headerHeight + (scaledHeight - headerHeight) / 2
    draw.SimpleText("Güçlü bir düşmana hazır olun!", "ZB_HomicideMedium", centerX, infoY, textColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
