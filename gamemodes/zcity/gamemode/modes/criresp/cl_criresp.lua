...
local teams = {
	[0] = {
		objective = "Müzakereler başarısız oldu, tehdidi etkisiz hale getirin. 10-4",
		name = "SWAT Operatifi",
		color1 = Color(68, 10, 255),
		color2 = Color(68, 10, 255)
	},
	[1] = {
		objective = "Bu benim evim, istediğimi yapabilirim.",
		name = "Şüpheli",
...
function MODE:HUDPaint()
	if zb.ROUND_START + 90 > CurTime() then
		...
		draw.SimpleText( "SWAT geliyor: "..string.FormattedTime(zb.ROUND_START + 90 - CurTime(), "%02i:%02i"	), "ZB_HomicideMedium", sw * 0.02 + posadd, sh * 0.95, Color(0,0,0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		draw.SimpleText( "SWAT geliyor: "..string.FormattedTime(zb.ROUND_START + 90 - CurTime(), "%02i:%02i"	), "ZB_HomicideMedium", (sw * 0.02) - 2 + posadd, (sh * 0.95) - 2, color, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		local fade = math.Clamp(zb.ROUND_START + 7.5 - CurTime(), 0, 1)
		surface.SetDrawColor(0, 0, 0, 255 * fade)
		surface.DrawRect(-1, -1, ScrW() + 1, ScrH() + 1)
	end
	...
	if zb.ROUND_START + 8.5 > CurTime() then
		...
		draw.SimpleText("Kriz Müdahalesi", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0, 162, 255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
CreateEndMenu = function(whowin)
	...
	surface.DrawText(ply:Name() .. (ply:GetNetVar("handcuffed", false) and " - etkisiz" or (not ply:Alive() and " - öldü") or ""))
	surface.SetFont("ZB_InterfaceMediumLarge")
	surface.SetTextColor(col.r, col.g, col.b, col.a)
