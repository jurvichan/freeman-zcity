...
local vgui_color_warning = Color(0, 100, 180, 255)
	...
	if(ply:Alive())then
		if(ply.isTraitor)then
			if(ply.SubRole == "traitor_infiltrator" or ply.SubRole == "traitor_infiltrator_soe")then
				local text = "(BASILI TUT)[ALT + E] Boyun Kır"
				...
				if(IsValid(aim_ent))then
					if(aim_ent:IsRagdoll())then
						local text = "[ALT + R] Görünüş Değiştir"
						local tw, th = surface.GetTextSize(text)
						local cx, cy = trace.HitPos:ToScreen().x, trace.HitPos:ToScreen().y
			...
			if(ply.SubRole == "traitor_assasin" or ply.SubRole == "traitor_assasin_soe" or ply.PlayerClassName == "sc_infiltrator")then
				...
				local text = "(BASILI TUT)[ALT + E] Silahsızlandır"
				local tw, th = surface.GetTextSize(text)
				local cx, cy = trace.HitPos:ToScreen().x, trace.HitPos:ToScreen().y
				cy = cy + y_offset
...
hook.Add("HUDPaint", "DrawTraitorPanel", function()
    ...
    local title = is_main and "ANA HAİN" or "HAİN YARDIMCISI"
    ...
    draw.SimpleText("Paneli açıp kapatmak için F4'e basın", "TraitorPanelText", x + traitor_panel.width/2, y + 42, 
                    Color(180, 180, 180, 220), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    
    local word_y = y + 65
    draw.SimpleText("Gizli Kelimeler:", "TraitorPanelText", x + traitor_panel.width/2, word_y, 
    ...
    if is_main then
        ...
        if has_assistants then
            draw.SimpleText("Yardımcıların:", "TraitorPanelText", x + traitor_panel.width/2, assist_y, 
                            Color(220, 220, 220), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            ...
            for _, traitor_info in ipairs(MODE.TraitorsLocal) do
                ...
                local status = is_alive and "" or " [ÖLÜ]"
            ...
            draw.SimpleText("Müsait yardımcı yok", "TraitorPanelText", x + traitor_panel.width/2, assist_y, 
