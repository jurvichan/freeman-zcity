...
MODE.Types.standard = {
	...
	PoliceEquipment = function(ply)
		...
		zb.GiveRole(ply, "Polis Memuru", Color(15,15,255))
	end
}
MODE.Types.wildwest = {
	Chance = 0.05,
	...
	LootTable = MODE.LootTableStandard,
	Messages = {
		[3] = "Ölü sessizlik boş şehri kaplıyor...",
		[1] = "Kasaba suçun eline düştü.",
		[0] = "Düzen yeniden sağlandı. Suçlu buydu:",
	},
	Message = "Suçlu: ",
	TraitorLoot = function(ply)
...
MODE.Types.soe = {
	...
	PoliceText = "Milli muhafızlar geldi.",
...
function MODE:RoundThink()
	...
	if self.Type ~= "soe" and not self.PoliceSpawned and self.saved.PoliceTime < CurTime() then
		...
		if max > 0 then
			...
			if spawned > 0 then
				...
				PrintMessage(HUD_PRINTTALK, "Polis geldi.")
	...
	if self.Type ~= "soe" and not self.swatDeployed and self.deadPoliceCount >= (self.spawnedPoliceCount or 4) and self.spawnedPoliceCount > 0 then
		...
		timer.Create("HMCDSpawnSWAT", 60, 1, function()
			...
			if count > 0 then
				PrintMessage(HUD_PRINTTALK, "SWAT timi geliyor!")
	...
	if self.Type == "soe" and not self.PoliceSpawned and self.saved.PoliceTime < CurTime() then
		...
		if count > 0 then
			...
			if spawned > 0 then
				self.PoliceSpawned = true
				PrintMessage(HUD_PRINTTALK, self.Types[self.Type].PoliceText or "Milli muhafızlar geldi.")
...
function MODE:EquipSWAT(ply, index)
    ...
    zb.GiveRole(ply, "SWAT Operatifi", Color(30, 30, 100))
end
...
function MODE:EquipNationalGuard(ply, index)
    ...
    local hands = ply:Give("weapon_hands_sh")
    ply:SetActiveWeapon(hands)
    zb.GiveRole(ply, "Milli Muhafız", Color(60, 90, 0))
end
...
hook.Add("PlayerCanPickupWeapon", "HMCD_TraitorRadioPickup", function( ply, weapon )
    if ply.isTraitor and weapon:GetClass() == "weapon_walkie_talkie" then
        if ply:HasWeapon("weapon_walkie_talkie") then
			...
			ply:ChatPrint("Ek telsizi sakladınız.")
        end
    end
end)
...
function MODE:RoundStart()
	...
	if(roles_choose)then
		...
		PrintMessage(HUD_PRINTTALK, "Katil " .. MODE.RoleChooseRoundStartTime .. " saniye boyunca rol seçiyor")
	else
		MODE.ChoosingPlayersList = {}
...
function MODE:EndRound()
	...
	if self.Type then
		if(MODE.RoleChooseRound)then
			if(winner ~= 1)then
				PrintMessage(HUD_PRINTTALK, "Tüm katiller durduruldu.")
				...
				PrintMessage(HUD_PRINTTALK, "Tüm masumlar öldürüldü.")
			end
			
			timer.Simple(2, function()
				if(players_alive == 0)then
					PrintMessage(HUD_PRINTTALK, "Kimse hayatta kalmadı.")
				else
					if(players_alive == 1)then
						PrintMessage(HUD_PRINTTALK, "Şehirde yalnızca 1 hayatta kalan kaldı.")
					else
						PrintMessage(HUD_PRINTTALK, players_alive .. " hayatta kalan şehirde kaldı.")
					end
				end
			end)
		else
...
hook.Add("Player_Death", "HMCD_PlayerDeath", function(ply, _)
	...
	timer.Simple(.1,function()
		...
		if ply.isTraitor then
			...
			if biggest_attacker == ply:Name() then
				ply:ChatPrint("İntihar ettiniz.")
			elseif not biggest_attacker then
				ply:ChatPrint("Öldünüz.")
			else
				ply:ChatPrint("Sizi öldüren: "..biggest_attacker..".")
			end
		end
	end)
end)
