...
function MODE:HUDPaint()
	...
	if StartTime + 20 > CurTime() then
		...
		draw.SimpleText( "Satın alma menüsü için F3'e bas", "ZB_HomicideMedium", sw * 0.5, sh * 0.9, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	else
...
local function PaintPanel1(self,w,h)
    ...
    surface.SetDrawColor(0, 100, 180, 55)
...
local function PaintPanel2(self,w,h)
    ...
    --surface.DrawRect( 0, 0, w, h, 2.5 )
	--draw.RoundedBox( 0, 2.5, 2.5, w-5, h-5, Color( 0, 0, 0, 140) )
    surface.SetDrawColor(155, 0, 0, 20)
    surface.SetMaterial(gradient_l)
...
local function OpenBuyMenu()
	...
	Frame:Center()
	Frame:MakePopup()
	Frame:SetTitle("Satın Alma Menüsü")
	...
	function lbl:Think()
		...
		self:SetText("Kalan Süre: "..string.FormattedTime(StartTime + 40 - CurTime(), "%02i:%02i:%02i"))
	end
	...
	function lbl:Think()
		self:SetText("Para: $"..LocalPlayer():GetNWInt("TDM_Money",0))
	end
