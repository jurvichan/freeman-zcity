	...
	timer.Simple(0.1,function()
		...
		for _, ply in player.Iterator() do
			...
			if ply:Team() == 1 then
				...
				zb.GiveRole(ply, "Karşı-Terörist", Color(0,0,190))
				ply:SetNetVar("CurPluv", "pluvberet")
			else
				ply:SetPlayerClass("terrorist")
				zb.GiveRole(ply, "Terörist", Color(190,0,0))
...
net.Receive("tdm_buyitem",function(len,ply)
	...
	if ((zb.ROUND_START or 0) + 40 < CurTime()) then ply:ChatPrint("Süre doldu!") return end
	...
	if tItem[3] then
		if not ply:HasWeapon(item.ItemClass) then ply:ChatPrint("Bu aksesuar için önce silah almanız gerekiyor.") return end
		if ((ply:GetNWInt("TDM_Money",0) - AttachmentPrice) < 0) then ply:ChatPrint("Yetersiz para.") return end
	...
	if ((ply:GetNWInt("TDM_Money",0) - item.Price) < 0) then ply:ChatPrint("Yetersiz para.") return end
