...
if CLIENT then
	...
	local colRed = Color(0, 100, 180, 255)
	...
	OpenInv = function(ent)
		...
		local name = nameStr .. " - Envanter" or "Konteyner"
		...
		plyMenu.PaintOver = function(self, w, h)
			...
			draw.DrawText("R - Kapat | LMB - Al | RMB - Eşya menüsü", "HomigradFontSmall", w / 2, h - h*0.055 , clr_text, TEXT_ALIGN_CENTER)
		end
		...
		for tab, things in pairs(inv) do
			...
			for k, i in ipairs(keys) do
				...
				button.DoClick = function()
					...
					if not functions[tab](ply, ent, i, unpack(thing1)) then
						local OptionsMenu = DermaMenu() 
							OptionsMenu:AddOption( "Bu eşyaya zaten sahipsiniz", function() end )
						OptionsMenu:Open()
						return
					end
				...
				button.DoRightClick = function()
					...
					--button:Remove()
					local OptionsMenu = DermaMenu() 
						OptionsMenu:AddOption( "Al", function() button:Remove() TakeItem(tab, i, thing, ent) end )
					OptionsMenu:Open()
					--timer.Simple(0.5 * math.max(ply:Ping() / 50,1),function()
					--	--OpenInv(ent)
					--end)
				...
				button.Paint = function(self, w, h)
					...
					surface.SetDrawColor(0, button.col1 * 0.4, button.col1, 15)
					...
					surface.SetDrawColor(0, button.col1 * 0.4, button.col1, button.col1)
