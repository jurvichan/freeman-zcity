...
local function CreateRadialMenu(options_arg, bAutoClose)
	...
	menuPanel.Paint = function(self, w, h)
		...
		if !paining then
			...
			draw.SimpleText(walkBtn .. " | Çeşitli", "HomigradFont", scrW * (0.981 + (0.04 * (1-viewLerp))),scrH * 0.9615, colBack, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
			draw.SimpleText(walkBtn .. " | Çeşitli", "HomigradFont", scrW * (0.98 + (0.04 * (1-viewLerp))),scrH * 0.96, colWhite, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		end
	end
end
...
hook.Add("radialOptions", "77", function()
	...
	if not organism.otrub and IsValid(lply:GetActiveWeapon()) and lply:GetActiveWeapon():GetClass() ~= "weapon_hands_sh" and lply:KeyDown(IN_WALK) then
		local tbl = {dropWeapon, "Silahı Bırak"}
