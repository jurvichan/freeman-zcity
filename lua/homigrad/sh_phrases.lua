...
local contextPhrasesTranslation = {
	["Scared"]       = "Korkmuş",
	["Yell"]         = "Bağır",
	["Cheer"]        = "Tezahürat",
	["Satisfied"]    = "Memnun",
	["Yell for help"]= "Yardım İste",
	["Someone died"] = "Biri Öldü",
	["Die!"]         = "Öldür!",
}
					...
					if mouseClick == 1 or organism.pain > 60 then
						...
						for context, phrases in pairs(contextPhrases[1]) do
							...
							tbl[#tbl + 1] = {
								...
								[2] = contextPhrasesTranslation[context] or context
							}
						end
						hg.CreateRadialMenu(tbl)
					end
				end,
				[2] = organism.pain > 60 and (organism.pain <= 100 and "Acıyla Bağır" or "İnle") or (lply.PlayerClassName == "furry" and "Miyavla") or "Bir Şey Söyle\nSAĞ TIK - Menü"
			}
		end
	end)
end
