...
local lowoxy = {
	"Az kalsın bayılacaktım... Yeterince oksijen yok.",
	"Yeterince oksijen yok... Daha fazla dayanamam...",
	"Gerçekten temiz havaya ihtiyacım var...",
	"Nefesim kesiliyor...",
	"Nefes almam lazım... yoksa buracıkta bayılacağım..."
}
...
local drop_mask = {
	"Bu maskede nefes alamıyorum... Çıkarmam lazım.",
	"Bırak şu maskeyi, değmez...",
	"İğrenç şey... bunda nefes almak imkânsız...",
	"Berbat kokuyor... Şu maskeyi çıkarmam şart...",
}

local drugged = {
	"Ohhh hahahaaa çoookk güüzel.....",
	"Mükkeemmmell..... hhissseediyyorumm iyiii..",
	"İişttee bbu oooluuu kkardeşşş",
	"Şu aN hissettiğimiN nE olduğunu gerçekteN seviyorumm....",
	"Ohhh evett buu çoookk iyiii hissettiriyoor!",
	"Hayatımın gERRRri kalanında böyle hissetmek istiyorum",
	"Ben burada nee yapıyorum ki?.. nNeeyse hehh",
	"Sseen kimmsin? Çekkilll giiitt...",
	"Başka bir şey istemiyorum... bu mükEMMMel!..",
}
...
module[2] = function(owner, org, timeValue)
	...
	if org.isPly and not org.otrub and o2.curregen < losing_oxy and org.analgesia <= 1.5 and !org.heartstop then
		if mask_blevota then
			if o2[1] < 15 then
				org.owner:Notify("O MASKEYI HEMEN ÇIKAR", 25, "take_gasmask2", 0, nil, color_red2)
			else
		...
		if o2[1] < 12 then
			...
			if o2[1] < 6 then
				org.owner:Notify("Oksijen... lütfen...", 30, "lowoxy2", 0, nil, color_red)
			end
		end
	end
	...
	if org.isPly then
		if org.pneumothorax > 0 then
			org.owner:Notify("Akciğerlerimde bir şeylerin dolduğunu hissediyorum.", true, "pneumothorax1",10) // delay of 10 seconds before typing that
		else
			org.owner:ResetNotification("pneumothorax1")
		end

		if org.pneumothorax > 0.3 then
			org.owner:Notify("Nefes almak giderek zorlaşıyor.", true, "pneumothorax2", 5)
		else
			org.owner:ResetNotification("pneumothorax2")
		end

		if org.pneumothorax > 0.5 then
			org.owner:Notify("Gerçekten nefes almakta çok zorlanıyorum.", true, "pneumothorax3", 5)
		else
	...
	if org.isPly then
		if org.brain > 0.1 and org.brain < 0.3 then
			org.owner:Notify(math.random(2) == 1 and "Başım çok ağrıyor..." or "Neredeyim?", true, "brain", 5)
		else
