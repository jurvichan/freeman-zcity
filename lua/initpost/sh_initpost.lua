	...
	hook.Add("OnEntityCreated", "doorInstructions", function(ent)
		if SDOIsDoor(ent) then
			if CLIENT then
				...
				"<font=ZCity_Tiny>"..string.upper( use ).." normal aç</font>\n"..
				"<font=ZCity_Tiny>"..string.upper( walk ).." + ".. string.upper( use ) .." yavaş aç</font>\n"..
				"<font=ZCity_Tiny>"..string.upper( speed ).." + ".. string.upper( use ) .." hızlı aç</font>\n"

				ent.HudHintMarkup = markup.Parse("<font=ZCity_Tiny>Kapı</font>\n<font=ZCity_SuperTiny><colour=125,125,125>".. ent.HowToUseInstructions .."</colour></font>", 450)
				ent.AdditionalInfoFunc = function()
					return lply:KeyDown(IN_WALK) and "Kapı açılıyor: "..math.Round(100 - math.min(math.abs(lply:EyeAngles().p) / 60, 1) * 100).."%" or ""
				end
			end
		end
	end)
