local PANEL = {}
local curent_panel 
local red_select = Color(0,120,200)

local Selects = {
    {Title = "Sunucudan Ayrıl", Func = function(luaMenu) RunConsoleCommand("disconnect") end},
    {Title = "Ana Menü", Func = function(luaMenu) gui.ActivateGameUI() luaMenu:Close() end},
    ...
    {Title = "Başarımlar", Func = function(luaMenu,pp)
        hg.DrawAchievmentsMenu(pp)
    end},
    {Title = "Ayarlar", Func = function(luaMenu,pp)
        hg.DrawSettings(pp)
    end},
    {Title = "Görünüm", Func = function(luaMenu,pp) hg.CreateApperanceMenu(pp) end},
    {Title = "Geri Dön", Func = function(luaMenu) luaMenu:Close() end},
}
...
local splasheh = {
    ...
    'LULU ÖLMEDI | !PLUV',
    'KATİL ÖLDÜRÜLDÜ',
    'ACEMİ HOMİCİDE SUNUCUSU',
    'BUNU DA DENE: MODDED HOMICIDE 2',
    'Z-CITY\'YE KATIL',
    'JOHN Z-CITY',
    ':pluvrare:',
    'SAW51 GERÇEK',
    'DAHA FAZLA SMALLTOWN',
    'DAHA FAZLA CLUE2022',
    'BACKROOMS == CLUE',
    'CEHENNEM YAKIN',
    'SAĞLIĞIN İYİ OLSUN, JASON STATHAM'
}
...
function PANEL:Init()
    ...
    zteam:SetText("Yapımcılar: uzelezz, Sadsalat, \nMr.Point, Zac90, Deka, Mannytko")
